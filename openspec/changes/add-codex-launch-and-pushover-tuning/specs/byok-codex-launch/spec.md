...
- **THEN** the `codex` child process is started with `BYOK_CODEX_API_KEY=sk-xxxx` in its environment and command-line `--config` overrides setting `model="gpt-4o"`, `model_provider="byok"`, `model_providers.byok.base_url="https://api.openai.com/v1"`, and `model_providers.byok.env_key="BYOK_CODEX_API_KEY"`
