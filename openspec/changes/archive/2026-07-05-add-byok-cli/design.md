...
`byok launch copilot --model gemma4` 會以旗標值覆寫 profile 的 default_model。未指定旗標時使用 profile 的 default_model。
...
- `byok launch copilot --model gemma4` 啟動的子行程環境中 `COPILOT_MODEL` 為 `gemma4`、`COPILOT_PROVIDER_BASE_URL` 為 profile 中 api_base（可透過 `byok launch copilot --dry-run` 或測試驗證）。
